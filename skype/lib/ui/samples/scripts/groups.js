// we want to dispose all the previous conversations added event listeners
// because in this demo site, we don't want to samples interfere with each
// other.
...
$(function () {
  ...
  var tagContactList = createGroupView(
      client.personsAndGroupsManager.all.persons, 'Contact List');
  ...
  function createGroupView(contacts, title) {
    ...
    contacts.added(function (contact) {
      var avatar = $("<div>").addClass("photo-c").
          append($("<img>").error(setDefaultAvatar));
      var detailPrimary = $("<div>").addClass("primary");
      var detail = $("<div>").addClass("detail").append(detailPrimary);
      var tagContact = $("<li>").addClass("persona").addClass("persona-small").
          append(avatar).append(detail).appendTo(tagGroup);
