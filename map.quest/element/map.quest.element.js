...
defineP([
  ...
  return MapQuestElement.extend('MapQuestElement', {
     ...
     */
    renderEmbeddedContent: function renderEmbeddedContent(key, zoom,
        zoomDblClick, location) {
      ...
      require(['http://www.mapquestapi.com/sdk/js/v7.2.s/mqa.toolkit.js?key=' +
      key], function _loadMap() {

        // create an object for options
        var options = {
          elt: $element.$[0],                         // ID of map element on
                                                      // page
          zoom: zoom,                                 // initial zoom level of
                                                      // the map
          latLng: {lat: center[0], lng: center[1]},   // center of map in
                                                      // latitude/longitude
          mtype: 'map',                               // map type (map, sat,
                                                      // hyb); defaults to map
          bestFitMargin: 0,                           // margin offset from map
                                                      // viewport when applying
                                                      // a bestfit on shapes
          zoomOnDoubleClick: !!zoomDblClick           // enable map to be
                                                      // zoomed in when
                                                      // double-clicking
        };
